...
def sync_caregiver(caregiver: schemas.CaregiverCreate, db: Session = Depends(get_db)):
    existing = db.query(models.Caregiver).filter(models.Caregiver.caregiver_id == caregiver.caregiver_id).first()
    if existing:
    ...
    db_caregiver = models.Caregiver(**caregiver.dict())
    db.add(db_caregiver)
    db.commit()
...
def link_caregiver_patient(link: schemas.CaregiverPatientLink, db: Session = Depends(get_db)):
    caregiver = db.query(models.Caregiver).filter(models.Caregiver.caregiver_id == link.caregiver_id).first()
    patient = db.query(models.Patient).filter(models.Patient.user_id == link.patient_id).first()
...
def create_game_session(session: schemas.GameSessionCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(models.GameSession).filter(models.GameSession.session_id == session.session_id).first()
...
def create_music_session(session: schemas.MusicSessionCreate, db: Session = Depends(get_db)):
    ...
    existing = db.query(models.MusicSession).filter(models.MusicSession.session_id == session.session_id).first()
    if existing: return existing

    db_session = models.MusicSession(**session.dict())
    db.add(db_session)
    db.commit()
    db.refresh(db_session)
    return db_session